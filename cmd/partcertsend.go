...
var partcertsendCmd = &cobra.Command{
	Use:   "partcertsend <event_name>",
	Short: "Dequeues events from RabbitMQ and sends participation certificates via email.",
	Long: `This command dequeues events from a RabbitMQ queue named "<event_name>dispatch_".
For each event, it constructs an email with a participation certificate attached.
The certificate file is expected to be found at @storage/cert/<event_name>/<student_email>.txt.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for d := range msgs {
			...
			for {
				...
				message.Attach(certPath, gomail.Rename("participation-certificate.pdf"))
...
func init() {
	rootCmd.AddCommand(partcertsendCmd)
}
